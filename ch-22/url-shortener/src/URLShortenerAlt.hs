...
routes = do 
    env <- ask
    lift $ get "/" (runReaderT actionHome env)
    lift $ get "/:short" (runReaderT actionShortUrl env)
    
actionHome :: ReaderT AppConfig ActionM ()
actionHome = do
    ioc <- asks conn  
    lift $
        queryParam "uri"  >>= 
...
actionShortUrl :: ReaderT AppConfig ActionM () 
actionShortUrl = do
    ioc <- asks conn  
    lift $
        pathParam "short"  >>= 
